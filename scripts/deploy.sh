...
#
# NOTE: kept intentionally plain ASCII. A Unicode char next to a $VAR once made
# bash read it as part of the variable name and abort under `set -u`.
#
...
# A push to the Pages source branch normally auto-builds, but we also POST an
# explicit build so the deploy is deterministic, then wait for it to go live.
if command -v gh >/dev/null 2>&1; then
  ...
  for _ in $(seq 1 24); do
    ...
    if [[ "${STATUS}" == built*"${TARGET}"* ]]; then echo " OK"; break; fi
    printf "."
    sleep 5
  done
  echo ">> Deployed. https://tonyanglesey.github.io/"
else
  echo "WARN: gh CLI not found - pushed gh-pages, but did not trigger a Pages build."
